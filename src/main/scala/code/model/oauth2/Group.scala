...
object Group extends Group with MongoMetaRecord[Group] {
  ...
  def searh(q: List[String]): JValue = {
    ...
    val db = Group.findAll(jmap, order, Skip(pageSize * (pageIndex - 1)), Limit(pageSize))
    val count = Group.count(qry)

    Message.returnMassage("group", "0", "Success", db.map(_.asJValue), count)
  }
  ...
  def update(q: JValue): JValue = {
    ...
    //    val avc = Group.findAll("_id" -> "a468451b-5faf-4c14-b389-bc1898dcaa87")
    //
    //    println(avc.size)
    for ((key, value) <- jsonmap) {
